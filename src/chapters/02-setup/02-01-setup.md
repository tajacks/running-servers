...
Configuration files are shown for reference, each followed by an `echo` command for quick provisioning.
...
{% codefile "/etc/apt/apt.conf.d/50unattended-upgrades" %}
...
{% codefile "/etc/apt/apt.conf.d/20auto-upgrades" %}
...
{% codefile "/home/$APP_USER/.config/containers/storage.conf" %}
[storage]
driver = "overlay"
